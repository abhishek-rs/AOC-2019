...
const mapOfWires = new Map();
const mapOfIntersections = new Map();

const checkAndAddToMaps = (tuple, map, intersectionMap, wireNo, noOfSteps) => {
  const tupleStr = tuple.join(",");
  const existingMapItem = map.get(tupleStr);
  if (existingMapItem && Number(existingMapItem[0]) !== wireNo) {
    intersectionMap.set(tupleStr, `${existingMapItem.slice(2)},${noOfSteps}`);
    map.set(tupleStr, `${wireNo},${noOfSteps}`);
  } else if (existingMapItem && existingMapItem[0] === wireNo) {
    return;
  } else {
    map.set(tupleStr, `${wireNo},${noOfSteps}`);
  }
...
const drawWirePath = (
  ...
  const [direction, size] = [instruction[0], Number(instruction.slice(1))];
  let endPosition;
  let noOfStepsCopy = noOfSteps;
  switch (direction) {
    case "R":
      for (let i = 1; i <= size; i++) {
        noOfStepsCopy = noOfStepsCopy + 1;
        checkAndAddToMaps(
          [currentPos[0] + i, currentPos[1]],
          map,
          intersectionMap,
          wireNo,
          noOfStepsCopy
      ...
      endPosition = [currentPos[0] + size, currentPos[1]];
      break;
    case "L":
      for (let i = 1; i <= size; i++) {
        noOfStepsCopy = noOfStepsCopy + 1;
    ...
    case "D":
      ...
      endPosition = [currentPos[0], currentPos[1] - size];
      break;
    case "U":
      for (let i = 1; i <= size; i++) {
        noOfStepsCopy = noOfStepsCopy + 1;
  ...
  return [endPosition, noOfStepsCopy];
};

const drawWire = (path, wireMap, intersectionMap, wireNo) => {
  let currentPos = [0, 0];
  let noOfSteps = 0;
  for (let i = 0; i < path.length; i++) {
    [currentPos, noOfSteps] = drawWirePath(
      path[i],
      currentPos,
      wireMap,
      intersectionMap,
...
const runProgram = (paths, wireMap, intersectionMap) => {
  drawWire(paths[0], wireMap, intersectionMap, 1);
  drawWire(paths[1], wireMap, intersectionMap, 2);
  //console.log(intersectionMap);
  console.log(
    "Answer: ",
    [...intersectionMap.keys()].reduce((acc, tuple) => {
      let val = tuple
        .split(",")
        .map(Number)
        .map(Math.abs)
...
const runProgram2 = (paths, wireMap, intersectionMap) => {
  drawWire(paths[0], wireMap, intersectionMap, 1);
  drawWire(paths[1], wireMap, intersectionMap, 2);

  console.log(
    "Answer: ",
    [...intersectionMap.values()].reduce((acc, tuple) => {
      let val = tuple
        .split(",")
        .map(Number)
        .reduce((sum, x) => x + sum, 0);
...
runProgram2(wirePaths, mapOfWires, mapOfIntersections);
